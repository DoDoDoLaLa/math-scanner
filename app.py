...
def render_howto_card(steps: List[str], *, title: str = "怎么用") -> None:
    items = "".join([f"<li>{stp}</li>" for stp in steps])
    st.markdown(
        f"<div class='hint-card'><b>{title}</b><ol>{items}</ol></div>",
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    st.divider()
    with st.expander("使用教程（各功能怎么用）", expanded=False):
        guide_tab = st.radio(
            "选择你要看的功能",
            ["Tab① PDF/图片 OCR→Markdown", "Tab② 公式OCR→LaTeX", "Tab③ Word→翻译/导出（含 Pandoc 中转）", "Tab④ Markdown→Word（可编辑公式）", "Tab⑤ 标题样式/字体/AI审查→导出"],
            index=0,
            key="sidebar_guide_tab",
        )
        if guide_tab.startswith("Tab①"):
            st.markdown("""
**操作步骤**
1. 上传 PDF 或图片（支持多页）。
2. 在侧边栏选择预设/参数（Max side / Tile height / Overlap / Tokens）。
3. 点击运行 OCR，得到 Markdown（含 $/$$ 公式）。
4. 需要的话下载 Word/LaTeX/Markdown（页面下方按钮）。
""")
        elif guide_tab.startswith("Tab②"):
            st.markdown("""
**操作步骤**
1. 上传一张“公式截图/裁剪图”。
2. 点击“识别公式并填入编辑器”。
3. 在编辑器里微调 LaTeX，并看实时预览。
4. 复制或用下载按钮导出 `formula.tex` / `formula_raw.txt`。
""")
        elif guide_tab.startswith("Tab③"):
            st.markdown("""
**操作步骤**
1. 上传 `.docx`。
2. 勾选需要的功能：翻译 / 公式替换 / $…$→Word公式。
3. 推荐开启：**Pandoc 中转翻译** + **AST 语义分块**（更稳，表格/列表不易被拆）。
4. 处理完成后下载译文 Word/Markdown/LaTeX。
""")
        elif guide_tab.startswith("Tab④"):
            st.markdown("""
**操作步骤**
1. 粘贴或上传 Markdown（含 $/$$ 公式）。
2. 选择是否把公式写回为可编辑 Word 公式（OMML）。
3. 导出 `.docx`。
""")
        else:
            st.markdown("""
**操作步骤**
1. 上传 `.docx`（建议是 Word 正确用了标题样式的文档）。
2. 选择 Heading 1/2/3 的字体/字号/加粗；必要时勾选应用到正文/表格/题注。
3. （可选）点击“运行 AI 审查”生成标题级别建议。
4. 点击“应用样式并导出”，然后在页面底部下载 `TitleFormatted.docx`。
""")
...
with tabs[0]:
    ...
    render_howto_card([
        "上传 PDF 或图片（PDF 支持多页）。",
        "在侧边栏选择预设并调整 OCR 参数（不懂可看侧边栏参数速查）。",
        "点击运行 OCR，得到 Markdown（含 $/$$ 公式）。",
        "根据需要下载 Markdown/Word/LaTeX。",
    ], title="Tab① 使用方法")
...
with tabs[1]:
    ...
    render_howto_card([
        ...
        "点击『识别公式并填入编辑器』。",
        "在编辑器中微调，查看实时预览。",
        "用下方按钮下载 formula.tex / formula_raw.txt。",
    ], title="Tab② 使用方法")
    ...
    st.download_button(
        '下载：formula_raw.txt（纯 LaTeX 主体）',
        data=(latex_text or '').encode('utf-8'),
        file_name='formula_raw.txt',
        mime='text/plain',
        key='dl_formula_raw',
    )
...
with tabs[2]:
    ...
    render_howto_card([
        "上传 .docx。",
        "按需勾选：翻译 / 公式替换 / $…$→Word公式。",
        "推荐开启：Pandoc 中转翻译 + AST 语义分块（表格/列表更稳）。",
        "处理完成后下载译文 Word / Markdown / LaTeX。",
    ], title="Tab③ 使用方法")
...
with tabs[3]:
    ...
    render_howto_card([
        "上传 .docx。",
        "选择导出格式：LaTeX / Markdown（或写回 Word）。",
        "需要统一样式时，可在相关页面上传 reference.docx。",
        "点击导出并下载生成文件。",
    ], title="Tab④ 使用方法")
...
with tabs[4]:
    ...
    render_howto_card([
        "上传 .docx（建议文档已正确使用 Heading 1/2/3 样式）。",
        "在下方设置标题字体/字号/加粗；需要时勾选应用到正文/表格/题注。",
        "可选：点击『运行 AI 审查』生成标题级别建议（用于纠正标题样式）。",
        "点击『应用样式并导出』，然后在页面底部下载 TitleFormatted.docx。",
    ], title="Tab⑤ 使用方法")
    ...
    # 如果换了文件，清空上一次的导出结果，避免 UI 混淆
    _new_name = getattr(docx_title_file, 'name', None) if docx_title_file else None
    if st.session_state.get('title_last_filename') != _new_name:
        st.session_state['title_last_filename'] = _new_name
        st.session_state.pop('title_formatted_docx', None)
        st.session_state.pop('title_rows', None)
        st.session_state.pop('ai_title_suggest', None)
    ...
    with st.expander("扩展应用范围（可选）", expanded=False):
        ...
        if st.button("应用样式并导出（可叠加 AI 建议）", type="primary", disabled=not docx_title_file, key="btn_apply_title_style"):
            try:
                ...
                st.session_state['title_formatted_docx'] = out_docx
            except Exception as e:
                st.error(f"导出失败：{type(e).__name__}: {e}")
    # --- persistent download (Streamlit rerun-safe) ---
    if st.session_state.get('title_formatted_docx'):
        st.download_button(
            '下载：TitleFormatted.docx',
            data=st.session_state['title_formatted_docx'],
            file_name='TitleFormatted.docx',
            mime='application/vnd.openxmlformats-officedocument.wordprocessingml.document',
            key='dl_title_formatted_persist',
        )